---
name: 电商商品套图生成
description: 先检查同一 SKU 参考图的视角覆盖，再通过需求访谈确认平台、受众、语言、套图类型、文案和风格，生成并逐图审计高一致性的电商白底图、首图、场景图、卖点图、细节图与尺寸图。适用于淘宝、京东、拼多多、抖音、小红书、Amazon 和独立站；不用于依据未展示的商品视角猜测外观，或伪造商品事实与授权。
slug: ecommerce-product-image-cn
displayName: 电商商品套图生成
version: 1.4.0
summary: 检查参考视角并确认套图方案，生成和审计高一致性商品图
license: MIT
---

# 电商商品套图生成

把商品图生产拆成参考证据检查、商品档案、套图计划、异步生成任务和逐图保真审计。默认使用算点边界 `Qx-Image`，支持最多 16 张同一 SKU 参考图。

## 对话门槛

- 不要在用户只说“做一套商品图”或只上传参考图时直接生成。先使用 [需求访谈指南](references/intake-guide.md) 收集缺失信息。
- 生成前必须按 [商品一致性指南](references/fidelity-guide.md) 检查每张参考图的商品版本和视角。不得生成参考证据未覆盖的商品正面、背面、侧面、结构或细节；例如只有衣服背面图时，必须索要正面图，不能猜正面。
...
- 最多使用 16 张参考图，但质量和视角覆盖优先于数量。所有参考图必须属于同一 SKU、颜色和版本，并标注 `front/back/side/detail/label` 等角色；不同颜色或款式分开生成。
...
- 自动审计只能检查尺寸、比例、文件可读性和白底边角。每张成图还必须与对应视角参考图并排检查轮廓、结构、颜色、图案、Logo、标签、接缝、纽扣、拉链、接口和配件；不一致的图片不得交付。

## 工作流

1. 按 [商品一致性指南](references/fidelity-guide.md) 检查素材并标注视角，再按 [需求访谈指南](references/intake-guide.md) 获取商品事实、目标平台、受众、场景、语言、套图类型、画面规格和品牌调性。素材覆盖不足时先索要图片，不进入计划。
2. 参考图可使用本地文件或模型服务能访问的 HTTPS URL。本地文件支持 PNG、JPG、JPEG、WEBP、GIF，单张不超过 10 MB；工具会按 [官方上传接口](https://token.qixuai.com/docs#image-upload)先上传，URL 有效期为 24 小时。参考图合计最多 16 张，每张必须使用 `ROLE=路径或URL` 标注视角。
...
4. 将用户确认的方案写入商品档案，再生成计划；这一步不调用模型。图型选择见 [references/image-types.md](references/image-types.md)：
   ...
   ```powershell
   python scripts/ecommerce_image_studio.py plan --product-name "便携咖啡杯" --category "饮具" --description "磨砂黑色杯身，不锈钢内胆，黑色旋盖" --selling-point "保温锁温" --selling-point "单手开合" --reference-url "front=https://example.com/product-front.jpg" --reference-url "detail=https://example.com/product-detail.jpg" --platform "京东" --audience "城市通勤人群" --scene "早晨办公桌" --language "简体中文" --text-mode render --types white_bg,hero,lifestyle,feature,detail --size 1:1 --output image_plan.json
   ```

   本地参考图使用 `--reference-file`，可重复传入；也可与 `--reference-url` 混用：

   ```powershell
   python scripts/ecommerce_image_studio.py plan --product-name "便携咖啡杯" --category "饮具" --description "磨砂黑色杯身，不锈钢内胆，黑色旋盖" --reference-file "front=.\product-front.jpg" --reference-file "detail=.\product-detail.png" --language "简体中文" --text-mode render --types white_bg,hero,lifestyle,feature,detail --size 1:1 --output image_plan.json
   ```
...
```text
image_plan.json: 商品档案、参考图角色与覆盖报告、目标语言、逐图文案、提示词、请求参数、预计积分
generation_manifest.json: 参考图角色、上传状态、URL 有效期、任务 ID、生成状态、结果 URL和本地文件
