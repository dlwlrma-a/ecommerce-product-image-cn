...
面向国内与跨境电商的高一致性商品套图 Skill。它先根据现有的同一 SKU 素材给出可执行方案，再把缺少的视角作为增强建议；确认平台、语言、逐图构图和准确文案后，通过算点边界 `Qx-Image` 直接生成并审计结果。

v1.9 改进了素材不足时的体验：

- 不要求先补齐所有视角；现有素材能安全支持的镜头可以直接生成。
- 缺少推荐视角时给出补图收益，不阻塞整套方案。
- 明确要求缺失视角的单张镜头会延后，其余镜头照常执行。
